...
//
// $Id: database-mysql.php,v 1.2 2001/04/19 13:24:45 edsonmedina Exp $
//
...
function db_connect()
{
    global $sys_dbhost,$sys_dbuser,$sys_dbpasswd,$conn,$conn_update,$sys_db_use_replication,$sys_dbreadhost;
...
{
    global $QUERY_COUNT,$sys_db_use_replication,$sys_db_is_dirty,$sys_dbname,$conn,$conn_update,$sys_dbhost,$sys_dbuser,$sys_dbpasswd;
    $QUERY_COUNT++;
...
function db_fetch_entry($field_name, $field_value,$table, $field_return)
{
    $textq = "SELECT $field_return FROM $table WHERE $field_name = $field_value";
    $query = db_query($textq);
    HandleMySqlErrorFatally("ERROR In fetch_entry QUERY: $textq SQL:");
    if (db_numrows($query)) $return = @mysql_result($query,0,0);
    else $return = "";
...
function db_fetch_entry2w($field_name1, $field_value1,$field_name2, $field_value2, $table, $field_return)
{
    $textq = "SELECT $field_return FROM $table WHERE $field_name1 = $field_value1 AND $field_name2 = $field_value2";    $query = db_query($textq);
    HandleMySqlErrorFatally("ERROR In fetch_entry2w QUERY: $textq SQL:");
    if (db_numrows($query)) $return = @mysql_result($query,0,0);
    else $return = "";
